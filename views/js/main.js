...
var resizePizzas = function(size) {
  ...
  function changePizzaSizes(size) {
    // Pizza elements are queried once and with getElementsByClassName instead of querySelectorAll
    var pizzas = document.getElementsByClassName("randomPizzaContainer"),
        sizePercentage = sizeToPercentage(size);
  ...
  function sizeToPercentage (size) {
    switch(size) {
      case "1":
        return 25;
      case "2":
...
// BACKGROUND PIZZAS POSITIONING
// Iterator for number of times the pizzas in the background have scrolled.
// Used by updatePositions() to decide when to log the average time per frame
var frame = 0;

// Background pizzas are queried once for all and stored
...
function updatePositions() {
  ...
  // Query scrollTop only once
  var scrollTop = document.body.scrollTop;
  // Store the 5 possible phase values in an array
  var phases = [0,1,2,3,4].map(function(remainder) {
...
// BACKGROUND PIZZAS GENERATION
document.addEventListener('DOMContentLoaded', function() {
  // Size of a pizza block in the background
  var s = 256;
  // The good number of pizzas is generated based on the window size
  var windowHeight = getWindowHeight();
  var rows = Math.ceil(windowHeight / s);
  var windowWidth = getWindowWidth();
  // +1 because there can be 2 little pizza block parts on each side of
  //   the screen
